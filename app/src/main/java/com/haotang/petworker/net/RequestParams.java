...
public class RequestParams implements Serializable {
	 ...
	 */
	public void put(String key, File files[], String contentType,
		...
		if (key != null) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				fileWrappers.add(new FileWrapper(files[i], contentType,
						customFileName));
			}
	...
	public String toString() {
		...
		for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileArrayParams
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
	...
	public HttpEntity getEntity(ResponseHandlerInterface progressHandler)
		...
		} else if (!forceMultipartEntity && streamParams.isEmpty()
				&& fileParams.isEmpty() && fileArrayParams.isEmpty()) {
	...
	private HttpEntity createMultipartEntity(
		...
		for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileArrayParams
			...
			for (FileWrapper fw : fileWrapper) {
				entity.addPart(entry.getKey(), fw.file, fw.contentType,
						fw.customFileName);
			}
		}
